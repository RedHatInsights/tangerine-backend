...
DEFAULT_URL = "http://localhost:3000/api/assistants/"
DEFAULT_SOURCE = "default"


def upload_files(source, directory_path, url, assistant_id, html, bearer_token):
    ...
    url = f"{url}/{assistant_id}/documents"
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Upload .rst/.md/.html/.adoc files to a tangerine assistant."
    )
    ...
    parser.add_argument("--assistant-id", type=int, help="assistant ID of the tangerine assistant.")
    parser.add_argument(
        "--source",
        type=str,
    ...
    parser.add_argument("--url", type=str, help="URL for assistants API", default=DEFAULT_URL)
    parser.add_argument("directory_path", type=str, help="Path to the directory containing files.")

    args = parser.parse_args()

    upload_files(
        args.source, args.directory_path, args.url, args.assistant_id, args.html, args.bearer_token
    )
